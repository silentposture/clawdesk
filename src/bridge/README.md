...
- [`scripts/prepare-host-agent-install-bundle.mjs`](../scripts/prepare-host-agent-install-bundle.mjs) turns that runtime into a concrete, self-contained install bundle with launcher, install/remove wrappers, scheduled-task registration, uninstall scripts, bundle-relative runtime paths, and an install step that can copy the bundle into a stable install root.
