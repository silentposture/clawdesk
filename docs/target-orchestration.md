...
- Reusable host bridge runtime plus local wrapper example that seeds a registry entry, persists host identity to a local config file by default, enforces a single-instance lock, writes a launcher status file for service-friendly supervision, and can be packaged into a self-contained install bundle that copies itself into a stable install root with launcher / uninstall scripts and bundle-relative runtime paths, redeems a host enrollment code, attests identity, and heartbeats against the gateway: [`src/bridge/host-bridge-agent.mjs`](../src/bridge/host-bridge-agent.mjs), [`src/bridge/host-agent-launcher.mjs`](../src/bridge/host-agent-launcher.mjs), [`scripts/prepare-host-agent-install-bundle.mjs`](../scripts/prepare-host-agent-install-bundle.mjs), [`examples/local-agent-bridge/bridge-agent.mjs`](../examples/local-agent-bridge/bridge-agent.mjs), [`scripts/verify-local-agent-bridge.mjs`](../scripts/verify-local-agent-bridge.mjs), [`scripts/verify-host-agent-config.mjs`](../scripts/verify-host-agent-config.mjs), [`scripts/verify-host-agent-lock.mjs`](../scripts/verify-host-agent-lock.mjs), [`scripts/verify-host-agent-launcher.mjs`](../scripts/verify-host-agent-launcher.mjs), [`scripts/verify-host-agent-install-bundle.mjs`](../scripts/verify-host-agent-install-bundle.mjs)
