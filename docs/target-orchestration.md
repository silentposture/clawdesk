...
- Reusable host bridge runtime plus local wrapper example that seeds a registry entry, persists host identity to a local config file by default, enforces a single-instance lock, redeems a host enrollment code, attests identity, and heartbeats against the gateway: [`src/bridge/host-bridge-agent.mjs`](../src/bridge/host-bridge-agent.mjs), [`examples/local-agent-bridge/bridge-agent.mjs`](../examples/local-agent-bridge/bridge-agent.mjs), [`scripts/verify-local-agent-bridge.mjs`](../scripts/verify-local-agent-bridge.mjs), [`scripts/verify-host-agent-config.mjs`](../scripts/verify-host-agent-config.mjs), [`scripts/verify-host-agent-lock.mjs`](../scripts/verify-host-agent-lock.mjs)
