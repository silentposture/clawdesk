...
- Mock gateway storage for registry, connection state, dispatch logs, per-target session timelines, and a toggleable target/global dispatch view in the UI: [`sidecars/mock-gateway/server.mjs`](../sidecars/mock-gateway/server.mjs), [`src/components/TargetRegistryPanel.tsx`](../src/components/TargetRegistryPanel.tsx)
