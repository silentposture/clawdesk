...
- Local host bridge example that seeds a registry entry, redeems a host enrollment code, attests identity, and heartbeats against the gateway: [`examples/local-agent-bridge/bridge-agent.mjs`](../examples/local-agent-bridge/bridge-agent.mjs), [`scripts/verify-local-agent-bridge.mjs`](../scripts/verify-local-agent-bridge.mjs)
