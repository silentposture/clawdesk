...
- Attest the host bridge identity after enrollment so the control plane can track device / install identity before connect.
...
- Keep a small local bridge example that can seed a target, redeem a host enrollment code, attest identity, and send heartbeat updates as a concrete stepping stone toward a real installed host bridge.
...
- Host bridge attestation that records device / install identity before connect and surfaces `attest` as the next action when a registered bridge has not yet attested: [`src/lib/targets.ts`](../src/lib/targets.ts), [`sidecars/mock-gateway/server.mjs`](../sidecars/mock-gateway/server.mjs), [`src/components/TargetRegistryPanel.tsx`](../src/components/TargetRegistryPanel.tsx), [`scripts/verify-host-enrollment.mjs`](../scripts/verify-host-enrollment.mjs)
...
- Reusable host bridge runtime plus local wrapper example that seeds a registry entry, persists host identity to a local config file by default, enforces a single-instance lock, writes a launcher status file for service-friendly supervision, and can be packaged into a self-contained install bundle that copies itself into a stable install root with launcher / uninstall scripts, bundle-relative runtime paths, and per-user startup-hook registration, redeems a host enrollment code, attests identity, and heartbeats against the gateway: [`src/bridge/host-bridge-agent.mjs`](../src/bridge/host-bridge-agent.mjs), [`src/bridge/host-agent-launcher.mjs`](../src/bridge/host-agent-launcher.mjs), [`scripts/prepare-host-agent-install-bundle.mjs`](../scripts/prepare-host-agent-install-bundle.mjs), [`examples/local-agent-bridge/bridge-agent.mjs`](../examples/local-agent-bridge/bridge-agent.mjs), [`scripts/verify-local-agent-bridge.mjs`](../scripts/verify-local-agent-bridge.mjs), [`scripts/verify-host-agent-config.mjs`](../scripts/verify-host-agent-config.mjs), [`scripts/verify-host-agent-lock.mjs`](../scripts/verify-host-agent-lock.mjs), [`scripts/verify-host-agent-launcher.mjs`](../scripts/verify-host-agent-launcher.mjs), [`scripts/verify-host-agent-install-bundle.mjs`](../scripts/verify-host-agent-install-bundle.mjs)
...
- SSH terminal reconnect lifecycle coverage is verified through a dedicated preflight gate that exercises open, command, reconnect, close, and relaunch paths against the gateway-managed shell stream: [`scripts/verify-ssh-terminal-lifecycle.mjs`](../scripts/verify-ssh-terminal-lifecycle.mjs), [`scripts/preflight.mjs`](../scripts/preflight.mjs)
...
5. Host bridge freshness is tracked through enrollment, attestation, and heartbeat so stale or unverified targets can be flagged before connect.
