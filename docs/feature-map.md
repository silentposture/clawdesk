...
- SSH terminal execution is available for allowlisted commands through the safe connector path, and SSH terminal sessions are available as a gateway-managed contract with redacted transcript snapshots, session summaries, and per-target timelines; remote desktop is currently a gateway-managed observe/control session contract with a gated native client launch helper, session summaries, and per-target timelines rather than a production transport.
