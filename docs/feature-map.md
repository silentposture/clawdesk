...
| Multi-target orchestration | Provides a target registry UI, pairing / host-key verification / connect actions, safe dispatch preview, gateway-managed SSH credential ref issuance, and allowlisted local-shell / SSH command execution under one control plane. SSH host-key verification persists a gateway-managed known_hosts entry before execution. Remote desktop sessions remain contract-only. | Implemented / Experimental | [`src/lib/targets.ts`](../src/lib/targets.ts), [`src/components/TargetRegistryPanel.tsx`](../src/components/TargetRegistryPanel.tsx), [`sidecars/mock-gateway/server.mjs`](../sidecars/mock-gateway/server.mjs), [`docs/target-orchestration.md`](target-orchestration.md) |
...
- SSH terminal execution is available for allowlisted commands through the safe connector path; interactive terminal sessions and remote desktop target adapters remain contract-only.
