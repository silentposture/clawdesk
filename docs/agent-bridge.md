...
- The install bundle generator at [`scripts/prepare-host-agent-install-bundle.mjs`](../scripts/prepare-host-agent-install-bundle.mjs) produces launcher, install/remove wrappers, scheduled-task registration, and uninstall scripts plus a manifest, so packaging can target the same runtime without inventing a second contract.
