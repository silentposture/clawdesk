...
- The install bundle generator at [`scripts/prepare-host-agent-install-bundle.mjs`](../scripts/prepare-host-agent-install-bundle.mjs) produces launcher, install/remove wrappers, scheduled-task registration, and uninstall scripts plus a manifest, and the generated bundle is self-contained: it carries a local copy of the reusable host bridge runtime and uses bundle-relative paths so the install root can be moved without depending on the repo checkout.
