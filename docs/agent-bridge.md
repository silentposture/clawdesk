...
- The install bundle generator at [`scripts/prepare-host-agent-install-bundle.mjs`](../scripts/prepare-host-agent-install-bundle.mjs) produces launcher, install/remove wrappers, startup-hook registration, and uninstall scripts plus a manifest, and the generated bundle is self-contained: it carries a local copy of the reusable host bridge runtime, uses bundle-relative paths, and can copy itself into a stable install root so the runtime can be installed without depending on the repo checkout.
