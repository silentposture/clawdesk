...
- Label: `feature`, `roadmap`
- Goal: unify safe dispatch across SSH terminals, remote desktop sessions, and local targets under a single control plane, with registry UI, pairing, gateway-managed host-key verification, gateway-managed SSH credential refs, allowlisted SSH/local-shell execution, gateway-managed remote-desktop observe/control session contracts, permission-gated control requests, and auditable dispatch records.
