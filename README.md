...
ClawDesk 是 OpenClaw-compatible、local-first 的 Windows 桌面 AI Agent。第一版使用 mock Gateway sidecar，因此可以先把桌面外殼、串流事件合約、Live Canvas、權限流程、商業授權、診斷與 Agent 管理做好，不需要先接 upstream OpenClaw repo。
...
- macOS/Linux 原生整合保留架構接口，不作為目前主要開發目標。
...
Microsoft Store readiness 候選 build：
...
Store 路徑使用獨立設定 `src-tauri/tauri.microsoftstore.conf.json`，保留 direct-download NSIS 設定不受影響。此設定使用 WebView2 offline installer mode、明確 publisher、legal resources、production CSP，並且不打包 mock Gateway resource。嚴格 Store 發佈檢查可用：
...
`授權` 面板已加入 Paddle + Keygen mock 架構：

- Paddle：mock 付款、訂閱、Lifetime、退款與支援更新續買事件。
- Keygen：mock license key、machine binding、裝置限制、離線票券、撤銷與篡改回報。
- 授權金鑰篡改、方案/更新日/裝置數修改或 license file 失效時，會降級 Hobby / safe mode，並顯示需要重新線上驗證。
- Windows machine fingerprint 只保存 salted hash，不保存明文 CPU/主機板序號。
- Lifetime Local 預設 USD $249，含 12 個月支援更新；更新維護續買 mock 價為 USD $75/年。

`授權` 面板同時顯示售價方案：

- Hobby：USD $0。
- Pro Monthly：USD $19/月。
- Pro Yearly：USD $190/年。
- Lifetime Local：USD $249。
- Team：USD $40/人/月。
- Enterprise：USD $60+/人/月或 USD $50,000/年起。
- BYOK Managed：USD $30/實例/月。

`版權` 面板顯示 ClawDesk 閉源商業授權、安裝同意條款、訂閱/自動續費揭露、OpenClaw-compatible 聲明、OpenClaw MIT 開源說明、第三方 NOTICE、隱私與使用者內容權利。使用者保留輸入、上傳檔案、專案資料與 AI 輸出內容權利；ClawDesk 不主張使用者內容所有權。

安裝與商業條款草案放在 `docs/legal/INSTALLER_TERMS.md`，並會被打包到 Tauri app resources 的 `legal/INSTALLER_TERMS.md`。OpenClaw MIT 與重製版權聲明放在 `docs/legal/OPENCLAW_MIT_NOTICE.md`，並會被打包到 `legal/OPENCLAW_MIT_NOTICE.md`。這兩份文件是產品落地草案，不構成法律意見；正式上架、收款或跨國販售前需由律師依實際銷售地區、Paddle/Keygen 合約與消費者保護法審閱。

`診斷` 面板會在本機建立非個資診斷包，使用者確認後才送出或匯出。診斷包不包含 Email、完整路徑、完整金鑰、API key、聊天內容、螢幕截圖或 Paddle customer id 明文，故障碼格式為 `CLWD-AREA-NNNN`。
...
`npm run release:guard` 是 mock 候選版檢查，會確認 ClawDesk 品牌、Tauri 版本、legal manifest、bundle resources 與可用 artifact 狀態。這個模式允許 mock Gateway、mock Paddle 與 mock Keygen，但報告會明確標示 `mock-candidate`，不能視為正式商業發佈。
...
strict 模式會要求 Paddle、Keygen、SSO 與 Windows code signing 相關環境變數；變數名稱請參考 `.env.production.example`。檢查報告只記錄環境變數是否存在，不會輸出 secret 值。
...
- Payment：Paddle API 與 webhook secret 是否存在。
...
這個指令會先執行 production release guard。只要仍打包 mock Gateway、缺少 Paddle/Keygen/SSO production credentials、或缺少 Windows signing 環境，就會在 build 前失敗，避免把 mock 候選版誤當正式商業版發佈。

Microsoft Store readiness 目前是第二條 Windows release lane：
...
這會依序執行 preflight、release guard、unit tests、build、MVP/backend 驗證、production preview GUI smoke、Cargo tests、Windows executable smoke 與 NSIS installer smoke。

## 後端服務數量與模擬部署

- **MVP 本機版**（最少後端）：`mock-gateway` 1 個服務即可。已覆蓋帳號、授權、MCP、工作流、記憶、診斷、人體工學與語音/影像能力驗證。
- **模擬完整授權與通知鏈路**：再加 `backend-auth`（模擬 Paddle/Keygen + 訂閱 webhook）與 `mock-mail`（驗證信預覽）與可選 `reverse-proxy`，總計 3～4 個服務。  
