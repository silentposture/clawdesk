...
Host enrollment registers the installed ClawDesk host bridge before pairing, and the bridge can continue sending heartbeat updates so the gateway can mark stale targets and recommend `heartbeat` when the bridge freshness ages out.
The host bridge can also submit attestation details with device / install identity so the readiness report can distinguish an enrolled bridge from a fully attested bridge and recommend `attest` when that identity has not yet been recorded.
...
Target registry 也支援可保存的 target groups / fleet presets，方便把常用的 SSH / remote-desktop / local-shell 組合直接套用到 broadcast selection，再搭配 batch execute 做一致派發。
...
`POST /targets/host-bridge/attest` 會記錄 host bridge device / install identity，並把 `attest` 納入 readiness / audit / timeline。
`POST /targets/host-bridge/heartbeat` 會更新 host bridge freshness，並把 stale bridges 標記回 readiness / audit / timeline。
